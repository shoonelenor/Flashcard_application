...
object DbSeeder {
    ...
    fun seedSubscriptionPlans(db: SQLiteDatabase) {
        ensureSubscriptionPlan(db, "Free",             "$0.00 / forever",  0,     "Access all free public decks and create up to 5 personal decks.",                                  10)
        ensureSubscriptionPlan(db, "Monthly Premium",  "$4.99 / month",    30,    "Full access to all premium decks, unlimited deck creation, and advanced stats.",                   20)
        ensureSubscriptionPlan(db, "Yearly Premium",   "$39.99 / year",    365,   "Best value. All premium features for a full year at a discounted price.",                         30)
        ensureSubscriptionPlan(db, "Lifetime",          "$99.99 once",      36500, "One-time payment for lifetime access to all current and future premium content.",                  40)
    }
    ...
    fun seedDemoDecksAndCards(db: SQLiteDatabase) {
        ...
        ensureDeckWithCards(db, noraId, "Math Formulas", "Useful formulas for quick revision.", isPublic = false, listOf(
            "Area of rectangle"   to "length x width",
            "Area of triangle"    to "1/2 x base x height",
            "Perimeter of square" to "4 x side",
            "Circumference"       to "2 x pi x r",
            "Pythagoras"          to "a^2 + b^2 = c^2",
            "Average"             to "sum / count",
            "Area of circle"      to "pi x r^2",
            "Simple Interest"     to "(P x R x T) / 100"
        ))
        ...
        // Premium decks
        premiumDecks().forEach { b -> ensurePremiumDeck(db, shoonId, b) }
        premiumDecks().forEach { b -> ensurePremiumDeck(db, noraId,  b) }
        premiumDecks().forEach { b -> ensurePremiumDeck(db, alexId,  b) }
        premiumDecks().forEach { b -> ensurePremiumDeck(db, leoId,   b) }
    }
    ...
    fun seedStudySessions(db: SQLiteDatabase) {
        ...
        repeat(8) { i -> seedSession(db, shoonId, now - i * day, DbContract.RESULT_KNOWN) }
        repeat(4) { i -> seedSession(db, shoonId, now - i * day, DbContract.RESULT_HARD)  }
        repeat(6) { i -> seedSession(db, noraId,  now - i * day, DbContract.RESULT_KNOWN) }
        repeat(3) { i -> seedSession(db, noraId,  now - i * day, DbContract.RESULT_HARD)  }
        repeat(5) { i -> seedSession(db, alexId,  now - i * day, DbContract.RESULT_KNOWN) }
        repeat(2) { i -> seedSession(db, alexId,  now - i * day, DbContract.RESULT_HARD)  }
        repeat(4) { i -> seedSession(db, mayaId,  now - i * day, DbContract.RESULT_KNOWN) }
        repeat(2) { i -> seedSession(db, mayaId,  now - i * day, DbContract.RESULT_HARD)  }
        repeat(3) { i -> seedSession(db, zinId,   now - i * day, DbContract.RESULT_KNOWN) }
        repeat(1) { i -> seedSession(db, zinId,   now - i * day, DbContract.RESULT_HARD)  }
        repeat(7) { i -> seedSession(db, leoId,   now - i * day, DbContract.RESULT_KNOWN) }
        repeat(3) { i -> seedSession(db, leoId,   now - i * day, DbContract.RESULT_HARD)  }
    }
    ...
    fun adminEnsurePremiumSeedContent(db: SQLiteDatabase): Int {
        ...
        try {
            listOf("shoon@gmail.com", "nora@gmail.com", "alex@gmail.com", "leo@gmail.com").forEach { email ->
                ...
                premiumDecks().forEach { b -> if (ensurePremiumDeck(db, uid, b)) count++ }
            }
    ...
    private fun premiumDecks(): List<SeedBundle> = listOf(
    ...
    private fun ensureCategory(
        db: SQLiteDatabase, name: String, description: String, sortOrder: Int
    ): Long {
        db.rawQuery(
    ...
    private fun ensureSubject(
        db: SQLiteDatabase, categoryId: Long, name: String, description: String, sortOrder: Int
    ) {
        db.rawQuery(
    ...
    private fun ensureSubscriptionPlan(
        db: SQLiteDatabase,
        name: String,
        priceText: String,
        durationDays: Int,
    ...
    private fun ensurePremiumDeck(
        db: SQLiteDatabase, ownerUserId: Long, bundle: SeedBundle
    ): Boolean {
        val existingId = db.rawQuery(
            "SELECT ${DbContract.D_ID} FROM ${DbContract.T_DECKS} WHERE ${DbContract.D_OWNER_USER_ID}=? AND ${DbContract.D_TITLE}=? LIMIT 1",
    ...
    private fun ensureFriendship(
        db: SQLiteDatabase, userId: Long, friendId: Long, status: String
    ) {
        if (userId < 0L || friendId < 0L) return
        val exists = db.rawQuery(
            "SELECT 1 FROM ${DbContract.T_FRIENDSHIPS} WHERE (user_id=? AND friend_id=?) OR (user_id=? AND friend_id=?) LIMIT 1",
    ...
    private fun seedSession(
        db: SQLiteDatabase, userId: Long, atTime: Long, result: String
    ) {
        if (userId < 0L) return
