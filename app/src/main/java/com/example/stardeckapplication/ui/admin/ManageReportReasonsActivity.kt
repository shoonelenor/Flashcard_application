...
class ManageReportReasonsActivity : AppCompatActivity() {
    ...
    private fun showCreateDialog() {
        ...
        buildReasonDialog(d, existing = null).show()
    }
    ...
    private fun showEditDialog(row: ReportReasonDao.ReportReasonRow) {
        ...
        buildReasonDialog(d, existing = row).show()
    }

    private fun buildReasonDialog(
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                if (name.length < 3) {
                    d.tilName.error = "At least 3 characters required"; ok = false
                } else if (name.length > 60) {
                    d.tilName.error = "Max 60 characters"; ok = false
                } else if (reportReasonDao.isNameTaken(name, existing?.id)) {
                    d.tilName.error = "Reason already exists"; ok = false
                }
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton("Delete") { _, _ ->
                ...
                if (rows == 1) {
                    ...
                    Snackbar.make(b.root, "Could not delete reason", Snackbar.LENGTH_LONG).show()
                }
            }
            .setNegativeButton("Cancel", null)
            .show()
    ...
    ) : ListAdapter<ReportReasonDao.ReportReasonRow, ReasonsAdapter.VH>(Diff()) {
