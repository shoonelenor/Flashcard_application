...
class UserStudyFragment : Fragment(R.layout.fragment_user_study) {
    ...
    private data class DeckStudyItem(
        val deck: StarDeckDbHelper.DeckRow,
        val totalCards: Int,
        val dueCount: Int,
    ...
    private fun reload() {
        ...
        executor.execute {
            val todayCount = runCatching { db.getTodayStudyCount(me.id) }.getOrDefault(0)
            val streakDays = runCatching { db.getStudyStreakDays(me.id) }.getOrDefault(0)
            val recent = runCatching { db.getRecentlyStudiedDeck(me.id) }.getOrNull()
            ...
            val items = decks.map { deck ->
                val totalCards = runCatching { db.getCardsForDeck(me.id, deck.id).size }.getOrDefault(0)
                val dueCount = runCatching { db.getDueCountForDeck(me.id, deck.id) }.getOrDefault(0)
            ...
            val totalDue = items.sumOf { it.dueCount }
            ...
            val recentTitle = recent?.title
            val recentMeta = recent?.lastStudiedAt?.let { "Last studied ${formatTime(it)}" }
    ...
    private fun bindTopArea(
        ...
        b.tvDueInfo.text = when {
            ...
            deckItems.isEmpty() -> {
                "No decks available yet. Create a deck in Library first."
            }

            else -> {
                "No cards are due right now. “Due Now” will fall back to your recent or first deck."
            }
        }
    ...
    private fun renderDeckCards(items: List<DeckStudyItem>) {
        ...
        items.take(8).forEach { item ->
            b.deckListContainer.addView(createDeckCard(item))
        }
    }
    ...
    private fun createDeckCard(item: DeckStudyItem): View {
        ...
        val desc = TextView(context).apply {
            text = item.deck.description?.takeIf { it.isNotBlank() }
                ?: "Open this deck in study mode."
            textSize = 14f
    ...
    private fun buildMetaText(item: DeckStudyItem): String {
        if (item.deck.isPremium && !isPremiumUser) {
            return "Premium deck • Unlock required"
        }
    ...
    private fun openBestDueStudy() {
        ...
        if (bestDueItem != null) {
            Snackbar.make(
                b.root,
                "Opening best due deck: ${bestDueItem.deck.title}",
        ...
        Snackbar.make(
            b.root,
            "No due cards yet. Opening ${fallback.deck.title} in normal review.",
            Snackbar.LENGTH_SHORT
        ).show()
